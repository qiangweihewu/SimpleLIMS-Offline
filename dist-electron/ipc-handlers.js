...
export function setupIpcHandlers() {
    ...
    ipcMain.handle('instrument:connect', async (_event, portPath, options) => {
        try {
    ...
    ipcMain.handle('instrument:disconnect', async (_event, portPath) => {
        await serialService.disconnect(portPath);
    });
    ipcMain.handle('instrument:getStatus', async (_event, portPath) => {
    ...
    ipcMain.handle('file:selectFile', async (_event, filters) => {
        const result = await dialog.showOpenDialog({
            properties: ['openFile'],
    ...
    ipcMain.handle('file:saveFile', async (_event, defaultPath, content) => {
    ...
    // ============= Backup/Restore =============
    ipcMain.handle('backup:create', async (_event, targetPath) => {
        return backupDatabase(targetPath);
    });
    ipcMain.handle('backup:restore', async (_event, sourcePath) => {
    ...
    ipcMain.handle('license:getStatus', async () => {
        const db = getDatabase();
        const licenseKey = db.prepare(`SELECT value FROM settings WHERE key = 'license_key'`).get();
        const activatedAt = db.prepare(`SELECT value FROM settings WHERE key = 'license_activated_at'`).get();
